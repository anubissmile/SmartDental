...
<html>
	<head>
		<title>Smart Dental:Recommended type</title>
	</head> 
	<body>
		<div class="uk-grid uk-grid-collapse">
			...
			<div class="uk-width-9-10">
				<%@include file="nav-top.jsp" %>
					...
					<form id="prenameFrm" action="SettingPersonRecommendedExcute" method="post">
					<div class="uk-grid ">
						<div class="uk-width-1-1 uk-overflow-container ">
							 <div class="uk-panel uk-panel-box uk-width-medium-1-1">
                                ...
                                <div class="uk-panel-header">
								    <h3 class="uk-panel-title"><i class="uk-icon-cog"></i> Recommended type</h3>
								</div>
								
								<div class="uk-grid uk-form "> 
									<div class="uk-form-icon uk-width-1-5"> 
										 <i class="uk-icon-asterisk"></i>
		                             		<input type="text" id="pre_name_id" name="recommendedModel.typerecommended" placeholder="รหัส" class="uk-width-1-1"
		                             			pattern="[0-9]{1,}" title="ใส่ได้เฉพาะตัวเลขท่านั้น" maxlength="4" required>
	                            	</div>
	                            	<div class="uk-form-icon uk-width-1-5"> 
										 <i class="uk-icon-asterisk"></i>
		                             		<input type="text" id="pre_name_th" pattern="[ก-๙A-Za-z ]{1,}" title="ใส่ได้เฉพาะตัวอักษรภาษาไทยและภาษาอังกฤษเท่านั้น" name="recommendedModel.typerecommended_name" placeholder="ช่องทางที่สมัคร" class="uk-width-1-1" required> 
	                            	</div>
                                ...
                                <div class="uk-panel-header">
									...
									<table class="uk-table uk-table-hover uk-table-striped uk-table-condensed border-gray " >
									    <thead>
									        <tr class="hd-table">
									            <th>รหัส</th>
									            <th>ช่องทางที่สมัคร</th>
									            <th></th> 
									        </tr>
									    </thead> 
									    <tbody>
									    	 <%   
											    if(request.getAttribute("recommendedList")!=null)	{
												    List recommendedList = (List) request.getAttribute("recommendedList");
				                                	List <RecommendedModel> recommendedModel = recommendedList;
				                                	int x=0;
					            	         	 	for(RecommendedModel pnmd : recommendedModel){ 
				            	         	 %>
										        <tr>
										            
										            <td class="uk-text-left"><%=pnmd.getTyperecommended()%></td>
										            <td class="uk-text-left"><%=pnmd.getTyperecommended_name()%></td>
										            <td class="uk-text-right">
										            	<a href="#update" 
										            	onclick="update('<%=pnmd.getTyperecommended()%>','<%=pnmd.getTyperecommended_name()%>')" 
										            	class="uk-button uk-button-primary uk-button-small" data-uk-modal>
															<i class="uk-icon-pencil"></i> แก้ไข
														</a>
										            	<a href="#delete_prename" 
										            	onclick="delete_prename('<%=pnmd.getTyperecommended()%>','<%=pnmd.getTyperecommended_name()%>')" 
