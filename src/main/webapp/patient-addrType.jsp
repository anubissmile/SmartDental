...
<html>
	<head>
		<title>Smart Dental:Type Address</title>
	</head> 
	<body>
		<div class="uk-grid uk-grid-collapse">
			...
			<div class="uk-width-9-10">
				...
				<div class="uk-grid"></div>
					<form id="formType" action="SettingPersonAddrTypeExcute" method="post">
					<div class="uk-grid ">
							 ...
							 <div class="uk-panel uk-panel-box uk-width-medium-1-1">
							 	<div class="uk-panel-badge uk-badge uk-badge-primary">master</div>
	                               <div class="uk-panel-header">
								    <h3 class="uk-panel-title"><i class="uk-icon-cog"></i> Address</h3>
								</div>
								<div class="uk-grid uk-form "> 
									<div class="uk-form-icon uk-width-1-5"> 
										<i class="uk-icon-asterisk"></i>
	                             		<input type="text" id="txt_id" name="addrModel.addr_typeid" placeholder="รหัสประเภท" class="uk-width-1-1"
	                             			pattern="[0-9]{1,4}" title="ใส่ได้เฉพาะตัวเลขเท่านั้น" maxlength="4" required>
	                            	</div>
	                            	<div class="uk-form-icon uk-width-1-5"> 
										<i class="uk-icon-asterisk"></i>
	                             		<input type="text" id="txt_name" pattern="[ก-๙A-Za-z]{1,}" title="ใส่ได้เฉพาะตัวอักษรภาษาไทยและภาษาอังกฤษเท่านั้น" name="addrModel.addr_typename" placeholder="ประเภทที่อยู่" class="uk-width-1-1" required> 
	                            	</div>
	                            	<div class="uk-form-icon uk-width-1-5">
							 	...
							 	<div class="uk-panel-badge uk-badge uk-badge-primary">list</div>
	                               <div class="uk-panel-header">
									...
									<table class="uk-table uk-table-hover uk-table-striped uk-table-condensed border-gray " >
									    ...
									    <tbody>
									    	 <%   
											    if(request.getAttribute("AddrTypeList")!=null)	{
												    List TelTypeList = (List) request.getAttribute("AddrTypeList");
				                                	List <AddressModel> AddrModel = TelTypeList;
				                                	int x=0;
					            	         	 	for(AddressModel pnmd : AddrModel){ %>
										        <tr>
										            <td class="uk-text-left"><%=pnmd.getAddr_typeid()%></td>
										            <td class="uk-text-left"><%=pnmd.getAddr_typename()%></td>
										            <td class="uk-text-right">
										            	<a href="#update" 
										            	onclick="update('<%=pnmd.getAddr_typeid()%>','<%=pnmd.getAddr_typename()%>')" 
										            	class="uk-button uk-button-primary uk-button-small" data-uk-modal>
															<i class="uk-icon-pencil"></i> แก้ไข
														</a>
										            	<a href="#delete_prename" 
										            	onclick="delete_prename('<%=pnmd.getAddr_typeid()%>','<%=pnmd.getAddr_typename()%>')" 
