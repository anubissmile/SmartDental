...
<html>
	<head>
		<title>Smart Dental:Type Tel</title>
	</head> 
	<body>
		<div class="uk-grid uk-grid-collapse">
			...
			<div class="uk-width-9-10">
				<%@include file="nav-top.jsp" %>
					...
					<form id="formType" action="SettingPersonTelTypeExcute" method="post">
					<div class="uk-grid ">
								...
								<div class="uk-grid uk-form "> 
									<div class="uk-form-icon uk-width-1-5"> 
										 <i class="uk-icon-asterisk"></i>
		                             		<input type="text" id="txt_id" name="telModel.tel_typeid" placeholder="รหัสประเภท" class="uk-width-1-1"
		                             			pattern="(?=.*\d).{1,}" title="กรุณาใส่รหัส" maxlength="4" required>
	                            	</div>
	                            	<div class="uk-form-icon uk-width-1-5"> 
										 <i class="uk-icon-asterisk"></i>
		                             		<input type="text" id="txt_name" name="telModel.tel_typename" placeholder="ชื่อประเภท" class="uk-width-1-1" required> 
	                            	</div>
	                            	<div class="uk-form-icon uk-width-1-5">
							...
							<div class="uk-panel uk-panel-box">
                                ...
                                <div class="uk-panel-header">
									...
									<table class="uk-table uk-table-hover uk-table-striped uk-table-condensed border-gray " >
									    ...
									    <tbody>
									    	 <%   
											    if(request.getAttribute("TypeTelList")!=null)	{
												    List TelTypeList = (List) request.getAttribute("TypeTelList");
				                                	List <TelephoneModel> TelModel = TelTypeList;
				                                	int x=0;
					            	         	 	for(TelephoneModel pnmd : TelModel){ 
				            	         	 %>
										        <tr>
										            
										            <td class="uk-text-left"><%=pnmd.getTel_typeid()%></td>
										            <td class="uk-text-left"><%=pnmd.getTel_typename()%></td>
										            <td class="uk-text-right">
										            	<a href="#update" 
										            	onclick="update('<%=pnmd.getTel_typeid()%>','<%=pnmd.getTel_typename()%>')" 
										            	class="uk-button uk-button-primary uk-button-small" data-uk-modal>
															<i class="uk-icon-pencil"></i> แก้ไข
														</a>
										            	<a href="#delete_prename" 
										            	onclick="delete_prename('<%=pnmd.getTel_typeid()%>','<%=pnmd.getTel_typename()%>')" 
