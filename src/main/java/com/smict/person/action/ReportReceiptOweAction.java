...
import com.smict.person.data.ReportReceiptOweData;
import com.smict.person.model.DepositModel; 

import ldc.util.Auth; 

public class ReportReceiptOweAction extends ActionSupport{
	
	ServicePatientModel servicePatModel; 
	DepositModel depositModel;
	private List<DepositModel> depositList;
	DepositData depositdb = new DepositData();  
	ReportReceiptOweData rro = new ReportReceiptOweData();
	/**
	 * CONSTRUCTOR
	 */
	public ReportReceiptOweAction(){
	...
	public String begin() throws Exception{
		...
		if(session.getAttribute("ServicePatientModel")!=null){ 
			...
			request.setAttribute("receiptList", rro.getOrder_list_receipt(hn));
			request.setAttribute("oweList", rro.getOrder_list_owe(hn));
		}
